        ...
        {user?.id === property.agentId && (
          <div className="card-actions-overlay">
            <button
              ...
              onClick={(e) => { e.stopPropagation(); navigate(`/post-property?edit=${propertyId}`); }}
              title="Edit Property"
            >
              <FiEdit2 size={16} />
            </button>
      ...
      <div className="property-info">
        ...
        {viewMode === "list" && (
          <div className="property-amenities">
            {(typeof property.amenities === 'string' ? property.amenities.split(',') : (property.amenities || [])).slice(0, 4).map((amenity, index) => (
              <span key={index} className={`amenity-tag ${typeof amenity === 'string' ? '' : 'placeholder'}`}>{String(amenity).trim()}</span>
            ))}
          </div>
        )}
      </div>
