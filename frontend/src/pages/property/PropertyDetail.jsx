...
import ProjectDetailView from "../../components/property/ProjectDetailView";
import "./PropertyDetail.css";
import { propertyApi, favoritesApi, userApi, chatApi, slotsApi, appointmentApi, reviewsApi, BASE_URL, IMAGE_URL } from "../../services/api";
...
function PropertyDetail() {
    ...
    const fetchProperty = async () => {
      try {
        ...
        const propertyData = {
          ...
          type: data.type,
          description: data.description || `Beautiful ${data.bhk} BHK ${data.type}. This property offers ${data.area} sq.ft.`,
          priceRaw: data.price,
          price: Math.round(data.price / 100000), // Lakhs
          pricePerSqft: data.area ? Math.round(data.price / data.area) : 0,
          area: data.area,
          bhk: data.bhk,
          age: data.age || "Ready to Move",
          address: data.address || data.location || "Prime Location",
          images: parsePropertyImages(data.photos).length > 0
            ? parsePropertyImages(data.photos).map(p =>
                p.startsWith("data:") || p.startsWith("http") ? p : `${IMAGE_URL}${encodeURIComponent(p)}`
              )
          ...
          agent: {
            ...
            profilePicture: data.agentProfilePicture,
          ...
          sold: data.isSold || data.sold || false,
  ...
  // Auto-play interval effect for Main Carousel - Transitions every 4 seconds
  useEffect(() => {
    if (!property?.images || property.images.length <= 1 || showLightbox) return;
  ...
  // --- Render Premium Project Layout ---
  if (property.type === "Projects" || property.type === "Project") {
    return (
      <ProjectDetailView
        property={property}
        user={user}
      ...
      <div className="property-detail-container">
        ...
        <div className="property-content">
          <div className="main-content">
            ...
            <div className="section location-details-section">
              <h3>Location Details</h3>
              <div className="location-info-card">
                <div className="location-row">
                  <span className="location-label">Full Address</span>
                  <span className="location-value">{property.address || "N/A"}</span>
                </div>
                <div className="location-row">
                  <span className="location-label">City</span>
                  <span className="location-value">{property.city}</span>
                </div>
                <div className="location-row">
                  <span className="location-label">Pincode</span>
                  <span className="location-value">{property.pinCode || "N/A"}</span>
                </div>
                {property.location && (
                  <div className="location-row">
                    <span className="location-label">Area</span>
                    <span className="location-value">{property.location}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="sidebar">
            <div className="agent-card">
              ...
              <div className="agent-actions">
                ...
                <a href={`tel:${property.agent.phone || '+919999999999'}`} className="contact-btn phone">
                  <PhoneIcon /> Call
                </a>
              </div>
            </div>
